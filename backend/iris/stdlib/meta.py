...
class LoadEnv(IrisCommand):
    title = "load environment from {filename}"
    examples = [ "load environment {filename}",
                 "load env from {filename}" ]
    help_text = [
        "This command loads an enviornment previously saved by Iris."
    ]
    def command(self, filename : t.File(question="What filename to load?", binary=True)):
        import dill as pickle
        # with open(name, 'rb') as f:
        data = pickle.loads(filename.content)
        self.iris.load_state(data)
        return "Loaded environment from \"{}\".".format(filename.name)
